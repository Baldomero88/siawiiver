...
#metodo para insertar los datos del Usuario
    ...
    public function iniciarSesion($sUsuario, $sPassword) {
        $sql = "SELECT nombreusuario, contrasena, Rol FROM usuario WHERE nombreusuario = '$sUsuario' AND contrasena = '$sPassword'";
        $result = mysqli_query($this->_dblink, $sql) or die('Error:' . mysqli_error($this->_dblink));
        if ($result->num_rows === 0) {
            return FALSE;
        }
        ...
        if ($this->aFields[0]['nombreusuario']) {
            $_SESSION['usuario'] = $this->aFields[0]['nombreusuario'];
            $_SESSION['rol'] = $this->aFields[0]['Rol'];
        } else {
            return FALSE;
        }
    }
